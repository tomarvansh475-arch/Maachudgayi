...
class PVPDatabase {
  ...
  async saveSiteContent(content: SiteContent) {
    ...
    try {
      const { error } = await supabase.from('pvp_site_content').upsert({ id: 'main', content, updated_at: new Date().toISOString() });
      if (error) {
        console.error("Supabase pvp_site_content upsert error:", error);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  async addVolunteer(volunteer: Omit<Volunteer, "id" | "createdAt" | "status"> & { status?: string }): Promise<Volunteer> {
    ...
    // Save to Supabase with proper timeout so we never block or lock the UI
    try {
      const insertPromise = supabase.from('pvp_volunteers').insert({
        id: newVolunteer.id,
      ...
      const { error } = await withTimeout(insertPromise, 2200, "Supabase volunteer insert timed out");

      if (error) {
        console.warn("Supabase insert failed. Saving to local database cache instead:", error);
      }
    } catch (e: any) {
      console.warn("Failed to sync volunteer record to Supabase, saved locally in-memory/cache:", e.message || e);
    }
  ...
  async addDonation(donation: Omit<DonationRecord, "status"> & { status?: string }): Promise<DonationRecord> {
    ...
    try {
      const { data, error } = await supabase.from('pvp_donations').insert({
      ...
      if (error) {
        console.warn("Supabase donation sync failed. Saving donation locally to memory/cache instead:", error);
      }
    } catch (e: any) {
      console.warn("Failed to sync donation record to Supabase, saved locally in-memory/cache:", e);
    }
